"""
analise_2016.py — Análise estatística de candidatos a vereador SP 2016 (TRE-SP).

Gera relatorio_2016.pdf com as análises da relação entre raça, receita de
campanha e desempenho eleitoral. Script autocontido (sem imports cruzados).

Particularidade de 2016: aproximadamente 52% das linhas têm RECEITA_TOTAL
ausente. As análises descritivas que não dependem dessa variável usam a
amostra completa; correlações, decis e regressões usam apenas o subconjunto
com receita declarada e essa perda é reportada no relatório.
"""
...
def preprocessar(df):
    ...
    log["n_receita_na"] = int(df["RECEITA_TOTAL"].isna().sum())
    log["pct_receita_na"] = log["n_receita_na"] / len(df) * 100 if len(df) else 0
...
def pagina_capa(pdf, log):
    ...
    info = (
        ...
        f"ATENÇÃO: {log['pct_receita_na']:.1f}% das linhas têm RECEITA_TOTAL ausente —\n"
        f"essas linhas são excluídas das análises financeiras e das regressões."
    )
    fig.text(0.5, 0.40, info, ha="center", fontsize=11,
             bbox=dict(boxstyle="round,pad=1", fc="#fff4e6", ec="#c98a3a"))
...
def pagina_caracterizacao(pdf, df, log):
    ...
    tab = pd.DataFrame({
        ...
        "Valor": [
            ...
            f"{log['n_nao_branco']:,}",
            f"{log['n_receita_na']:,}",
            f"{log['pct_receita_na']:.1f}%",
            f"{df['BENS_TOTAL'].isna().sum():,}",
        ],
    })
    add_table(fig, tab, rect=(0.10, 0.42, 0.80, 0.48), fontsize=9)
...
def pagina_correlacao(pdf, universo, titulo, secao):
    ...
    interp = (
        ...
        f"A correlação é mais forte entre {mais_forte}. Atenção: a alta taxa de NA "
        f"em RECEITA_TOTAL em 2016 (≈52%) reduz significativamente o N efetivo "
        f"e pode introduzir viés de seleção se a ausência não for aleatória."
    )
...
def pagina_decis_receita(pdf, df, secao):
    ...
    try:
        ...
        interp = (
            ...
            f"de o candidato ser competitivo. Decis calculados apenas sobre candidatos "
            f"com receita declarada (~48% da amostra em 2016)."
        )
    except Exception:
...
def pagina_sumario(pdf, sumario):
    ...
    interp = (
        ...
        "diferenças entre grupos raciais e entre modelos. Atenção ao N reduzido "
        "nas regressões em 2016 (consequência da alta taxa de NA em receita)."
    )
...
def main():
    ...
    print(f"NA em receita: {log['n_receita_na']} ({log['pct_receita_na']:.1f}%)")
