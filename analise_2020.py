"""
analise_2020.py — Análise estatística de candidatos a vereador SP 2020 (TRE-SP).

Gera relatorio_2020.pdf com as análises da relação entre raça, receita de
campanha e desempenho eleitoral. Script autocontido (sem imports cruzados).

Particularidade de 2020: a base introduz a categoria 'NÃO DIVULGÁVEL' em
DS_GENERO, DS_GRAU_INSTRUCAO, DS_ESTADO_CIVIL, DS_COR_RACA e ST_REELEICAO
(efeito LGPD). Essas linhas são excluídas das regressões mas mantidas nas
análises descritivas quando as variáveis envolvidas não as exigem.
"""
...
def preprocessar(df):
    ...
    log["n_nao_divulgavel_raca"] = int((df["DS_COR_RACA"] == "NÃO DIVULGÁVEL").sum())
    ...
    log["n_nao_divulgavel_genero"] = int(df["DS_GENERO"].isin(VALORES_NAO_DIVULGAVEIS).sum())
    log["n_nao_divulgavel_instrucao"] = int(df["DS_GRAU_INSTRUCAO"].isin(VALORES_NAO_DIVULGAVEIS).sum())
    log["n_nao_divulgavel_reeleicao"] = int(df["ST_REELEICAO"].isin(VALORES_NAO_DIVULGAVEIS).sum())
...
def pagina_capa(pdf, log):
    ...
    info = (
        ...
        f"Linhas 'NÃO DIVULGÁVEL' em raça: {log['n_nao_divulgavel_raca']}"
    )
...
def pagina_caracterizacao(pdf, df, log):
    ...
    tab = pd.DataFrame({
        "Métrica": [
            ...
            "NA em RECEITA_TOTAL",
            "NA em BENS_TOTAL",
            "'NÃO DIVULGÁVEL' em DS_GENERO",
            "'NÃO DIVULGÁVEL' em DS_GRAU_INSTRUCAO",
            "'NÃO DIVULGÁVEL' em ST_REELEICAO",
        ],
        "Valor": [
            f"{log['n_inicial']:,}",
            f"{log['n_apos_dropna']:,}",
            f"{log['n_excluidos_raca']:,}",
            f"{log['n_nao_divulgavel_raca']:,}",
            ...
            f"{log['n_nao_divulgavel_genero']:,}",
            f"{log['n_nao_divulgavel_instrucao']:,}",
            f"{log['n_nao_divulgavel_reeleicao']:,}",
        ],
    })
    add_table(fig, tab, rect=(0.10, 0.40, 0.80, 0.50), fontsize=8)

    dist = pd.Series(log["dist_raca_original"]).sort_values(ascending=False)
    ax = add_chart(fig, rect=(0.14, 0.08, 0.76, 0.26))
...
# ============================================================
def df_para_modelo(df, cols_obrigatorias, cols_categoricas):
    """Remove NAs e linhas com 'NÃO DIVULGÁVEL' em qualquer categórica do modelo."""
    sub = df.dropna(subset=cols_obrigatorias).copy()
    for col in cols_categoricas:
        sub = sub[~sub[col].isin(VALORES_NAO_DIVULGAVEIS)]
    return sub
...
def ajustar_ols(df):
    ...
    cats = ["DS_GENERO", "DS_GRAU_INSTRUCAO", "ST_REELEICAO"]
    sub = df_para_modelo(df, cols, cats)
...
def coletar_sumario(df, competitivos, eleitos, correl_todos, correl_comp,
    ...
    for nome, mod in [("OLS todos", ols_todos), ("OLS competitivos", ols_comp)]:
        if mod is not None and "LOG_RECEITA" in mod.params.index:
            add(f"{nome}: efeito log(receita)", "Coef. (p)",
                f"{mod.params['LOG_RECEITA']:.3f} (p={mod.pvalues['LOG_RECEITA']:.3g})")

    for nome, mod in [("Logit todos", logit_todos), ("Logit competitivos", logit_comp)]:
        if mod is not None and "LOG_RECEITA" in mod.params.index:
            add(f"{nome}: efeito log(receita) [odds]", "exp(coef)",
                f"{np.exp(mod.params['LOG_RECEITA']):.3f} (p={mod.pvalues['LOG_RECEITA']:.3g})")
